...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    var mainViewController: PresentationEditorViewController?
    ...
    var editNameButton: UIBarButtonItem?
    ...
    convenience init(mainViewController: PresentationEditorViewController, leftMenuViewController: UIViewController) {
        self.init()
    ...
    convenience init(mainViewController: PresentationEditorViewController, rightMenuViewController: UIViewController) {
        self.init()
        self.mainViewController = mainViewController
    ...
    convenience init(mainViewController: PresentationEditorViewController, leftMenuViewController: UIViewController, rightMenuViewController: UIViewController) {
        self.init()
        self.mainViewController = mainViewController
        leftViewController = leftMenuViewController
        rightViewController = rightMenuViewController
    ...
    func changeMainViewController(_ mainViewController: PresentationEditorViewController,  close: Bool) {
...
extension SlideMenuController {
    ...
    func initNavigationBar() {
        // add edit bg button
        let editBgButton = UIBarButtonItem.init(image: UIImage.init(named: "icon_edit_bg.png"), style: .plain, target: self, action: #selector(editBgButtonClicked(barButton:)))
        let seperateView1 = UIBarButtonItem.init(title: "|", style: .plain, target: self, action: nil)

        if (self.mainViewController?.isComeFromTemplate)! {
            // init all icon for template
            deleteButton = UIBarButtonItem(image: UIImage(named: "icon_delete.png"),  style: .plain, target: self, action: #selector(deleteButtonClicked(barButton:)))
            saveButton = UIBarButtonItem(image: UIImage(named: "icon_save.png"),  style: .plain, target: self, action: #selector(saveButtonClicked(barButton:)))
//            let updateDesignToCloudButton = UIBarButtonItem(image: UIImage(named: "icon_update_design_cloud"),  style: .plain, target: self, action: #selector(updateDesignToCloudButtonClicked(barButton:)))
            let menuButton = UIBarButtonItem(image: UIImage(named: "icon_menu.png"),  style: .plain, target: self, action: #selector(menuButtonClicked(barButton:)))
            
//            navigationItem.rightBarButtonItems = [menuButton, updateDesignToCloudButton, saveButton, deleteButton]
            navigationItem.rightBarButtonItems = [menuButton, saveButton!, deleteButton!, seperateView1, editBgButton]

        } else {
            // init all icons for presentation
            deleteButton = UIBarButtonItem(image: UIImage(named: "icon_delete.png"),  style: .plain, target: self, action: #selector(deleteButtonClicked(barButton:)))
            saveButton = UIBarButtonItem(image: UIImage(named: "icon_save.png"),  style: .plain, target: self, action: #selector(saveButtonClicked(barButton:)))
            sendButton = UIBarButtonItem(image: UIImage(named: "icon_send.png"),  style: .plain, target: self, action: #selector(sendButtonClicked(barButton:)))
            editNameButton = UIBarButtonItem.init(image: UIImage.init(named: "icon_edit"), style: .plain, target: self, action: #selector(editNameButtonClicked(barButton:)))
            let menuButton = UIBarButtonItem(image: UIImage(named: "icon_menu.png"),  style: .plain, target: self, action: #selector(menuButtonClicked(barButton:)))
            let seperateView2 = UIBarButtonItem.init(title: "|", style: .plain, target: self, action: nil)

            navigationItem.rightBarButtonItems = [menuButton, sendButton!, seperateView2, saveButton!, deleteButton!, seperateView1, editBgButton, editNameButton!]
        }
        
        // add custom back button
        navigationItem.hidesBackButton = true
        let newBackButton = UIBarButtonItem(image: UIImage.init(named: "icon_arrow_left"), style: .plain, target: self, action: #selector(handleBackButton(sender:)))
        navigationItem.leftBarButtonItem = newBackButton
    }
    
    func deleteButtonClicked(barButton: UIBarButtonItem) {
        self.mainViewController?.deleteButtonClicked(barButton: barButton)
    }
    
    func saveButtonClicked(barButton: UIBarButtonItem) {
        self.mainViewController?.saveButtonClicked(barButton: barButton)
    }
    
    func sendButtonClicked(barButton: UIBarButtonItem) {
        self.mainViewController?.sendButtonClicked(barButton: barButton)
    }
    
    func menuButtonClicked(barButton: UIBarButtonItem) {
        slideMenuController()?.toggleRight()
    }
    
    func updateDesignToCloudButtonClicked(barButton: UIBarButtonItem) {
        self.mainViewController?.updateDesignToCloudButtonClicked(barButton: barButton)
    }
    
    func editBgButtonClicked(barButton: UIBarButtonItem) {
        self.mainViewController?.editBgButtonClicked(barButton: barButton)
    }
    
    func editNameButtonClicked(barButton: UIBarButtonItem) {
        self.mainViewController?.editNameButtonClicked(barButton: barButton)
    }
    
    func handleBackButton(sender: UIBarButtonItem) {
        self.mainViewController?.handleBackButton(sender: sender)
    }
}

// MARK: - PresentationEditorViewControllerDelegate

extension SlideMenuController: PresentationEditorViewControllerDelegate {
    
    func handleAccessRightForNavigationBar(accessRight: Int) {
        if (self.mainViewController?.isComeFromTemplate)! {
            if !AccessRightManager.canClone(accessRight: accessRight) {
                saveButton?.isEnabled = false
            }
            if !AccessRightManager.canUpdate(accessRight: accessRight) {
            ...
            if !AccessRightManager.canUse(accessRight: accessRight) {
