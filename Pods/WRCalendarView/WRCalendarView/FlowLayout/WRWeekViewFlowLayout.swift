...
class WRWeekViewFlowLayout: UICollectionViewFlowLayout {
    ...
    func layoutItemsAttributes(section: Int, sectionX: CGFloat, calendarStartY: CGFloat) {
        ...
        for item in 0..<collectionView!.numberOfItems(inSection: section) {
            ...
            guard let (startHourY, startMinuteY, endHourY, endMinuteY) = getStartAndEndPositionY(at: itemIndexPath, section: section) else {
                continue
            }
    ...
    func getStartAndEndPositionY(at itemIndexPath: IndexPath, section: Int) -> (CGFloat, CGFloat, CGFloat, CGFloat)? {
        guard let calendarType = delegate?.getCalendarType() else {
            return nil
        }
        
        var startHourY: CGFloat = 0
        var startMinuteY: CGFloat = 0
        var endHourY: CGFloat = 0
        var endMinuteY: CGFloat = 0
        
        switch calendarType {
        case .week, .day:
            let itemStartTime = startTimeForIndexPath(itemIndexPath)
            let itemEndTime = endTimeForIndexPath(itemIndexPath)
            let dayForSection = daysForSection(section)
            
            // set up for startTimeY
            if (itemStartTime.year! < dayForSection.year!) || (itemStartTime.month! < dayForSection.month!) || (itemStartTime.day! < dayForSection.day!) {
                startHourY = 0
                startMinuteY = 0
            } else {
                startHourY = CGFloat(itemStartTime.hour!) * hourHeight
            ...
            if itemEndTime.day! != itemStartTime.day! {
                if itemEndTime.day! == dayForSection.day! {
        ...
        case .weekly:
            let event = eventForIndexPath(itemIndexPath)
            
            startMinuteY = CGFloat(event.startTime) * minuteHeight
            endMinuteY = startMinuteY + (CGFloat(event.duration) * minuteHeight)
        }
        
        return (startHourY, startMinuteY, endHourY, endMinuteY)
    }
    ...
    func eventForIndexPath(_ indexPath: IndexPath) -> WREvent {
        if let event = delegate?.collectionView(collectionView!, layout: self, eventForItemAtIndexPath: indexPath) {
            return event
        } else {
            fatalError()
        }
