...
class WREventCell: UICollectionViewCell {
    ...
    var event: WREvent? {
        didSet {
            if let event = event {
                ...
                bgColor = UIColor.init(hexString: event.color)
                updateColors()
            }
        }
    }
    ...
    func backgroundColorHighlighted(_ selected: Bool) -> UIColor {
        if bgColor != nil {
            return selected ? bgColor! : bgColor!.withAlphaComponent(0.1)
        }
        return selected ? UIColor.init(r: 53, g: 177, b: 241) : UIColor.init(r: 53, g: 177, b: 241).withAlphaComponent(0.1)
    }
    
    func textColorHighlighted(_ selected: Bool) -> UIColor {
        return selected ? UIColor.white : UIColor.init(r: 33, g: 114, b: 156)
    }
